...
// Monthly revenue
$stmt = $conn->prepare("SELECT DATE_FORMAT(booking_date, '%Y-%m') AS month, SUM(total_price) AS monthly_revenue FROM booked_events GROUP BY month");
$stmt->execute();
$monthly_revenue = $stmt->get_result()->fetch_all(MYSQLI_ASSOC);

// Latest approved events
$stmt = $conn->prepare("SELECT event_name, event_date FROM events WHERE pending_approval = 0 ORDER BY event_date DESC LIMIT 5");
$stmt->execute();
$latest_events = $stmt->get_result()->fetch_all(MYSQLI_ASSOC);

// Total bookings for each event and average rating
$stmt = $conn->prepare("SELECT e.event_id, e.event_name, IFNULL(SUM(b.quantity), 0) AS total_bookings, IFNULL(AVG(r.rating), 0) AS average_rating, COUNT(r.review_id) AS total_reviews, IFNULL(SUM(b.total_price), 0) AS total_revenue FROM events e LEFT JOIN booked_events b ON e.event_id = b.event_id LEFT JOIN event_reviews r ON e.event_id = r.event_id GROUP BY e.event_id, e.event_name");
$stmt->execute();
$events_data = $stmt->get_result()->fetch_all(MYSQLI_ASSOC);

// Top performing event
$stmt = $conn->prepare("SELECT event_name, SUM(quantity) AS total_bookings FROM booked_events JOIN events ON booked_events.event_id = events.event_id GROUP BY booked_events.event_id ORDER BY total_bookings DESC LIMIT 1");
$stmt->execute();
$top_event = $stmt->get_result()->fetch_assoc();

// Least performing event
$stmt = $conn->prepare("SELECT event_name, SUM(quantity) AS total_bookings FROM booked_events JOIN events ON booked_events.event_id = events.event_id GROUP BY booked_events.event_id ORDER BY total_bookings ASC LIMIT 1");
$stmt->execute();
$least_event = $stmt->get_result()->fetch_assoc();

// Event approval rate
$stmt = $conn->prepare("SELECT (SUM(CASE WHEN pending_approval = 0 THEN 1 ELSE 0 END) / COUNT(*)) * 100 AS approval_rate FROM events");
$stmt->execute();
    ...
    <main class="admin-container">
        ...
        <div class="grid-container">
            <section class="stats">
                ...
                <div class="stat-item">
                    <h3>Total Users <i class="fa fa-users"></i></h3>
            ...
            <section class="approval-rate">
                ...
                <p><?php echo number_format($approval_rate, 2); ?>%</p>
            </section>
